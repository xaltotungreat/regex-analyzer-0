...
import org.eclipselabs.real.gui.e4swt.conveyor.stage.SubmitSearchStageCR;
import org.eclipselabs.real.gui.e4swt.conveyor.stage.VerifyParamsStageCR;
import org.eclipselabs.real.gui.e4swt.parts.GUISearchResult;

public class ComplexRegexStageBuilder implements IStageTreeBuilder {


    @Override
    public IConveyorStage buildStageTree(ConvSearchRequest req) {
        IConveyorStage root = new RootStage();
        // first create the GUI formatter
        ...
        if (req.isSearchInCurrent()) {
            ...
            formatter = new GUIUpdater.IStatusFormatter() {
                ...
                public void updateStatus(GUISearchResult partSRObj, ISearchProgressMonitor searchMonitor, String searchID) {
                    partSRObj.setSearchStatus(searchID, "Searched files " + searchMonitor.getCompletedSOFiles()
                            + "/" + searchMonitor.getTotalSOFiles() + " Found objects " + searchMonitor.getObjectsFound(),
                            (searchMonitor.getCompletedSOFiles() != null)?searchMonitor.getCompletedSOFiles():0,
                            (searchMonitor.getTotalSOFiles() != null)?searchMonitor.getTotalSOFiles():0);
                }
            };
        }
        ...
        IConveyorStage executionPoint = root.addChild(new CreateSearchInfoStage()).addChild(new ParamsDialogStage()).
                addChild(new VerifyParamsStageCR()).
        ...
        IConveyorStage afterResultPoint = executionPoint.
                addChild((req.isSearchInCurrent()?(new SubmitSearchCurrentStage()):(new SubmitSearchStageCR()))).
                addChild(processRes);
